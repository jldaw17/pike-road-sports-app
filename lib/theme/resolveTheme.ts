...
export function resolveAthleticOSTheme(
  ...
  const preset =
    ...
    power5Theme;
  const isLockedStyleTheme =
    preset.key === 'clean_slate' || preset.key === 'modern' || preset.key === 'premium';
  ...
  const resolvedStyles = isLockedStyleTheme
    ? { ...preset.styles }
    : {
        backgroundStyle:
